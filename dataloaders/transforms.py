...
class DropBeams(object):
    
    def __init__(self):
        self.cam_p = np.array([[7.215377e+02, 0.000000e+00, 6.095593e+02, 4.485728e+01],
    ...
    def get_depth_point_cloud(self, depth_map, cam_p, min_v=0, flatten=True, in_cam0_frame=True):
    ...
    def project_depths(self, point_cloud, cam_p, image_shape, max_depth=100.0):
    ...
    def drop_beams(self, points):
        pts_depth = np.linalg.norm(points, axis=1)
        pts_depth = np.expand_dims(pts_depth, axis=1)
        normalized_points = points / pts_depth

        beams_nb = 46
        squash_nb = 1
        z_partitions = np.linspace(np.min(normalized_points[:,2]), np.max(normalized_points[:,2]), beams_nb+1)
        z_partitions[0] -= 1.0
        z_partitions[beams_nb] += 1.0
        selected_points = None

        for i in range(z_partitions.shape[0]-1):
            l, h = z_partitions[i], z_partitions[i+1]
            partition_idxs = np.where(np.logical_and(normalized_points[:,2]>=l, normalized_points[:,2]<h))[0]
            if i % squash_nb == 0:
                partition_points = points[partition_idxs, :]
                if selected_points is None:
                    selected_points = partition_points
                else:
                    selected_points = np.append(selected_points, partition_points, axis=0)

        points = np.zeros_like(selected_points)
        points[:,0], points[:,1], points[:,2] = selected_points[:,1], selected_points[:,2], selected_points[:,0]
        points = points.T
    ...
    def __call__(self, depth_map):
        ...
        cam0_curr_pc = self.get_depth_point_cloud(depth_map, self.cam_p)
        valid_mask = (cam0_curr_pc[2] != 0)
        cam0_curr_pc = cam0_curr_pc[:, valid_mask].T
        points = np.zeros_like(cam0_curr_pc)
        points[:,0], points[:,1], points[:,2] = cam0_curr_pc[:,2], cam0_curr_pc[:,0], cam0_curr_pc[:,1]

        points = self.drop_beams(points)

        new_depth_map = self.project_depths(points, self.cam_p, depth_map.shape)
        new_depth_map = np.expand_dims(new_depth_map, axis=2)
